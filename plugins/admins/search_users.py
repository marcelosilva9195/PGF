...
async def search_users(c: Client, m: InlineQuery):
    ...
    if not user:
        results = [
            InlineQueryResultArticle(
                title="⚠️ Você deve especificar um nome, username ou ID.",
    ...
    if not users:
        results = [
            InlineQueryResultArticle(
                ...
                description=f"⚠️ Nenhum usuário encontrado para '{user}'.",
                input_message_content=InputTextMessageContent(
                    f"⚠️ Nenhum usuário encontrado para '{user}'."
                ),
            )
        ]
...
async def view_history(c: Client, m: CallbackQuery):
    ...
    history = cur.execute(
        "SELECT tipo, email, senha, cidade, bought_date FROM docs_sold WHERE owner = ?",
        [user_id],
    ).fetchall()
    if not history:
