...
class MisticPay:
    """Classe para pagamento via MisticPay (PIX automatico com QR Code)."""
    ...
    async def create_payment(
        ...
        rt = await hc.post(
            "https://api.misticpay.com.br/v1/pix/create",
            headers=headers,
            json=payload,
        )
        ...
        return {
            ...
            "copy_paste": rjson.get("pix_code"),
            "qr_code": rjson.get("qr_code_base64"),
            "status": rjson.get("status"),
        }
    ...
    async def verify(self):
        ...
        rt = await hc.get(
            f"https://api.misticpay.com.br/v1/pix/{self.payment_id}",
            headers=headers,
        )

        rjson = rt.json()

        if rjson.get("status") in ["paid", "PAGO", "approved", "CONCLUIDA"]:
            self.status_payment = "PAGO"
